...
export function SuggestionToast({
  ...
  return (
    <div
      className={cn(
        "fixed bottom-24 left-1/2 -translate-x-1/2 w-full max-w-md px-4",
    ...
    >
      <div className="bg-white/90 backdrop-blur-md rounded-xl border border-[#7a9f6a]/30 p-4 shadow-xl shadow-[#7a9f6a]/10">
        <div className="flex items-start gap-3">
          {/* Wingman icon with animation */}
          <div
            className={cn(
              "flex-shrink-0 w-10 h-10 rounded-full bg-[#7a9f6a]/20",
              "flex items-center justify-center",
              isWhispering && "animate-pulse"
            )}
          >
            {isWhispering ? (
              <Volume2 className="w-5 h-5 text-[#7a9f6a]" />
            ) : (
              <Lightbulb className="w-5 h-5 text-[#7a9f6a]" />
            )}
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            {isWhispering && (
              <p className="text-xs text-[#7a9f6a] mt-1 flex items-center gap-1">
                <span className="inline-block w-2 h-2 rounded-full bg-[#7a9f6a] animate-pulse" />
                Whispering...
              </p>
            )}
          </div>
